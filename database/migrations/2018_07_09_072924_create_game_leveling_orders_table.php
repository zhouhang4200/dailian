...
class CreateGameLevelingOrdersTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('game_leveling_orders', function (Blueprint $table) {
            ...
            $table->unsignedInteger('user_id')->comment('发单用户ID');
            $table->string('username', 80)->comment('发单用户名');
            $table->string('user_phone', 20)->default(0)->comment('发单用户电话');
            $table->string('user_qq', 20)->default(0)->comment('发单用户qq');
            $table->unsignedInteger('parent_user_id')->comment('发单单用户父ID');
            $table->string('parent_username', 80)->comment('发单单用户父用户名');
            $table->string('parent_qq', 20)->comment('发单单用户父用qq');
            $table->string('parent_phone', 20)->comment('发单单用户父电话');
            $table->unsignedInteger('take_user_id')->default(0)->comment('接单用户ID');
            $table->string('take_username', 80)->nullable()->comment('接单用户名');
            $table->string('take_user_qq', 20)->nullable()->comment('接单用户qq');
            $table->string('take_user_phone', 20)->nullable()->comment('接单用户手机号');
            $table->unsignedInteger('take_parent_user_id')->default(0)->comment('接单用户父ID');
            $table->string('take_parent_username', 80)->nullable()->comment('接单用户父用户名');
            $table->string('take_parent_user_qq', 20)->default(0)->comment('接单用户父qq');
            $table->string('take_parent_user_phone', 20)->nullable()->comment('接单用户父用户电话');
