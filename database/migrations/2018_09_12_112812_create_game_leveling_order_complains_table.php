...
class CreateGameLevelingOrderComplainsTable extends Migration {
	...
	public function up()
	{
		Schema::create('game_leveling_order_complains', function(Blueprint $table)
		{
			...
			$table->boolean('status')->default(1)->comment('状态 1 处理中 2 成功 3取消');
			$table->boolean('initiator')->comment('发起人 1 发单方 2 接单方');
			$table->timestamp('dispose_at')->default(DB::raw('CURRENT_TIMESTAMP'))->comment('处理时间');
			$table->timestamps();
		});
	}
	...
	{
		Schema::drop('game_leveling_order_complains');
	}
