@if(is_object($order) && in_array(optional($order->complain)->status, [1, 2]))
    <div class="layui-form">
        <table class="layui-table">
            ...
            <tbody>
            <tr>
                <td>{{ $order->complain->parent_user_id == request()->user()->parent_id ? '我' : '对方' }}</td>
    ...
    <div class="layui-form" style="margin-bottom: 18px;" id="message-list">
        <table class="layui-table">
            ...
            @forelse($order->message as $msg)
                <tr>
                    <td>{{ $msg->from_user_id == request()->user()->parent_id ? '我' : ($msg->initiator == 3 ? '客服' : '对方')}}</td>
                    <td>{{ $msg->content }}</td>
